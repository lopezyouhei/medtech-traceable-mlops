...
def run_cleaned_validation():
    ...
    df = pd.read_csv(cleaned_data_path, dtype=EXPECTED_SILVER_TYPES)
    ...
    try:
        data_asset = data_source.get_asset(name="cleaned_data_asset")
    except Exception:
    ...
    # define cleaned data expectations
    ## table properties
    validator.expect_table_columns_to_match_set(
        column_set=CATEGORICAL_SETS["SILVER_COLUMN_NAMES"]
    )

    ## type checks
    for feature, expected_type in EXPECTED_SILVER_TYPES.items():
    ...
    # TODO: add binarized NUM to data contract
    # validator.expect_column_values_to_be_in_set(
    ...
    val_def_name = "cleaned_data_val_def"
    try:
    ...
    checkpoint_name = "cleaned_data_checkpoint"
    try:
    ...
    # TODO: implement dvc hash collection from dvc.lock instead of from data folder as
    #  it will not be stored there anymore

    # # get dvc and git hash
    # dvc_file_path = cleaned_data_path.with_name(cleaned_data_path.name + ".dvc")
    # try:
    ...
    #     dvc_hash = f"untracked_{cleaned_data_path.name}"
    ...
    # logger.info(f"Running validation with DVC hash {dvc_hash} and Git hash {git_hash}.")
    ...
    # logger.info(f"View validation report at: {docs_url}")
